...
1) Language files installation -----------------------------------------
	...
	1.1) Copy qb64_UDL_v0.002.xml to the userDefineLangs folder.
	1.2) Copy QB64FL.xml to the functionList folder of NP++.
	1.3) Copy overrideMap.xml to the functionList folder or just add the code line:
		<association id="QB64FList.xml" userDefinedLangName="QB64"/>
	     as expleined inside the overrideMap.xml to the existing file if you have more than one UDL FunctionList rules.


2) Config NotePad to use F5 key to compile .bas file  -------------------

	2.1) Copy QB64_noIDE.exe to your QB64 folder, aside with QB64.exe.
	2.2) Configure manualy windows 10 to open all .bas files with QB64_noIDE.exe, selecting the "Open With.." option over any .bas file.
